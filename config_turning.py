...
class SimulationConfig:
    ...
    TURN_PROBABILITY: float = 0.35  # Chance to turn instead of going straight
    TURN_RADIUS: int = 40  # Radius of the turning arc (pixels)
    INTERSECTION_HALF_SIZE: int = 35  # Trigger zone half-size around the intersection center
    ...
    TRAFFIC_LIGHT_VEHICLE_VERTICAL_X: int = 355  # Для движения вниз/юг
    TRAFFIC_LIGHT_VEHICLE_VERTICAL_Y: int = 125
    TRAFFIC_LIGHT_VEHICLE_VERTICAL_ROTATION: int = 0

    TRAFFIC_LIGHT_VEHICLE_VERTICAL_OPPOSITE_X: int = 470  # Для движения вверх/север
    TRAFFIC_LIGHT_VEHICLE_VERTICAL_OPPOSITE_Y: int = 340
    TRAFFIC_LIGHT_VEHICLE_VERTICAL_OPPOSITE_ROTATION: int = 0

    TRAFFIC_LIGHT_VEHICLE_HORIZONTAL_OPPOSITE_X: int = 520  # Для движения влево/запад
    TRAFFIC_LIGHT_VEHICLE_HORIZONTAL_OPPOSITE_Y: int = 160
    TRAFFIC_LIGHT_VEHICLE_HORIZONTAL_OPPOSITE_ROTATION: int = 0
    
    TRAFFIC_LIGHT_PEDESTRIAN_HORIZONTAL_X: int = 353  # HORIZONTAL_CROSSWALK_X + CROSSWALK_WIDTH - 60
    TRAFFIC_LIGHT_PEDESTRIAN_HORIZONTAL_Y: int = 158  # HORIZONTAL_LANE_Y - 80
    TRAFFIC_LIGHT_PEDESTRIAN_HORIZONTAL_ROTATION: int = 0
    
    TRAFFIC_LIGHT_PEDESTRIAN_VERTICAL_X: int = 300  # VERTICAL_LANE_X - 70
    TRAFFIC_LIGHT_PEDESTRIAN_VERTICAL_Y: int = 305  # VERTICAL_CROSSWALK_Y + CROSSWALK_WIDTH + 10
