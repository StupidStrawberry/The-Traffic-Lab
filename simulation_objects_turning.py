...
# ----------------------------------------------------------------------
# NEW: Helper to build a two‑lane road using line.png tiles.
# Call this from your main simulation file instead of setting a static
# background image (bg main.png).
# ----------------------------------------------------------------------
def setup_two_lane_road(scene, config):
    """
    Adds a two‑lane road background to the given QGraphicsScene.
    The road is made of repeated 'line.png' tiles placed side by side.
    Assumes config contains SCENE_WIDTH, SCENE_HEIGHT, and road geometry.
    """
    line_pixmap = QPixmap("line.png")
    if line_pixmap.isNull():
        # Fallback: create a simple grey line if the image is missing
        line_pixmap = QPixmap(100, 20)
        line_pixmap.fill(Qt.GlobalColor.gray)

    # Example placement: two horizontal lanes covering the whole scene width
    # You can adjust Y positions based on your road layout.
    lane_y_positions = [config.SCENE_HEIGHT // 3, 2 * config.SCENE_HEIGHT // 3]

    for y in lane_y_positions:
        # Tile the line image across the scene width
        for x in range(0, config.SCENE_WIDTH, line_pixmap.width()):
            item = QGraphicsPixmapItem(line_pixmap)
            item.setPos(x, y - line_pixmap.height() // 2)
            scene.addItem(item)
