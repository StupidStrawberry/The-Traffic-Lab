...
class VehicleItem(QGraphicsPixmapItem):
    def __init__(self, vehicle: Vehicle, x: float, y: float, config):
        ...
        # Turning state (simple right-turn at the intersection)
        self.turning = False
        self._turn_cx = 0.0
        self._turn_cy = 0.0
    ...
    def _apply_rotation_from_vector(self, dx: float, dy: float) -> None:
        """Rotate pixmap to face the movement direction (dx, dy) in scene coordinates."""
    ...
    def _start_right_turn(self, target_dir: str) -> None:
        """Start a smooth 90° right turn using a quarter-circle arc."""
        self.turning = True
        self._turn_target_dir = target_dir
        r = float(getattr(self.config, 'TURN_RADIUS', 40))
        self._turn_r = r

        # Arc definition depends on current direction (screen y grows downward).
        if self.vehicle.direction == 'horizontal_right' and target_dir == 'vertical_down':
            # East -> South : theta -pi/2 .. 0, center at (ix, y + r)
            ix = self.config.VERTICAL_LANE_X
            y0 = self.y()
            self._turn_cx = float(ix)
            self._turn_cy = float(y0 + r)
            self._turn_theta = -math.pi / 2
            self._turn_theta_end = 0.0

        elif self.vehicle.direction == 'horizontal_left' and target_dir == 'vertical_up':
            # West -> North : theta pi/2 .. pi, center at (ix, y - r)
            ix = self.config.VERTICAL_LANE_X
            y0 = self.y()
            self._turn_cx = float(ix)
            self._turn_cy = float(y0 - r)
            ...
            # South -> West : theta 0 .. pi/2, center at (x - r, iy)
            iy = self.config.HORIZONTAL_LANE_Y
            x0 = self.x()
            self._turn_cx = float(x0 - r)
            self._turn_cy = float(iy)
            self._turn_theta = 0.0
            self._turn_theta_end = math.pi / 2

        elif self.vehicle.direction == 'vertical_up' and target_dir == 'horizontal_right':
            # North -> East : theta pi .. 3pi/2, center at (x + r, iy)
            iy = self.config.HORIZONTAL_LANE_Y
            x0 = self.x()
            self._turn_cx = float(x0 + r)
            self._turn_cy = float(iy)
            ...
            # Unsupported mapping -> do nothing
            self.turning = False
            self._turn_target_dir = None
            return

    def _update_turn(self) -> None:
        """Advance the turning arc by one tick."""
        r = max(1.0, float(self._turn_r))
        # Arc length step ~ speed pixels, so dtheta = s / r
        dtheta = float(self.speed) / r
        ...
        # Tangent vector for increasing theta: (-sin, cos)
        ...
        if abs(self._turn_theta - self._turn_theta_end) < 1e-6:
            ...
            # Snap rotation to canonical angles
    ...
    def _maybe_begin_turn(self) -> bool:
        """Randomly begin a right turn when entering the intersection trigger zone."""
        if self.turning:
            return False

        p = float(getattr(self.config, 'TURN_PROBABILITY', 0.35))
        if p <= 0.0:
            return False
        if random.random() >= p:
            return False

        half = float(getattr(self.config, 'INTERSECTION_HALF_SIZE', 35))
        ix = float(self.config.VERTICAL_LANE_X)
        iy = float(self.config.HORIZONTAL_LANE_Y)

        # Trigger conditions use vehicle "front" edge in its current direction.
        if self.vehicle.direction == 'horizontal_right':
            front_x = self.x() + self.config.VEHICLE_WIDTH
            if (front_x >= ix - half) and (self.x() <= ix):
                self._start_right_turn('vertical_down')
                return self.turning

        elif self.vehicle.direction == 'horizontal_left':
            front_x = self.x()
            if (front_x <= ix + half) and (self.x() >= ix):
                self._start_right_turn('vertical_up')
                return self.turning

        elif self.vehicle.direction == 'vertical_down':
            front_y = self.y() + self.config.VEHICLE_HEIGHT
            if (front_y >= iy - half) and (self.y() <= iy):
                self._start_right_turn('horizontal_left')
                return self.turning

        elif self.vehicle.direction == 'vertical_up':
            front_y = self.y()
            if (front_y <= iy + half) and (self.y() >= iy):
                self._start_right_turn('horizontal_right')
                return self.turning
    ...
    def move(self, vehicles: List['VehicleItem'], pedestrians: List['PedestrianItem'],
        ...
        # Possibly begin a turn when entering the intersection
        if self._maybe_begin_turn():
            self._update_turn()
            return False
