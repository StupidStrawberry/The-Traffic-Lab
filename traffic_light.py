...
class TrafficLightController:
    ...
    def update(self) -> None:
        ...
        if (self.timer >= self.config.VEHICLE_GREEN_TIME and self.timer <= self.config.VEHICLE_GREEN_TIME + self.config.VEHICLE_YELLOW_TIME) or self.timer >= self.config.TRAFFIC_LIGHT_CYCLE - 10:
            self.vehicle_green = False
            self.vehicle_yellow = True
            self.pedestrian_green = False
        elif self.timer < self.config.VEHICLE_GREEN_TIME:
            self.vehicle_green = True
            self.vehicle_yellow = False
            self.pedestrian_green = False
        else:
