...
from traffic_light import EditableCrosswalkItem, EditableSpawnPointItem, TrafficLightController, TrafficLightItem
...
class SimulationWidget(QGroupBox):
    ...
    EDITOR_LABELS = {
        ...
        'turn_point_horizontal_right': 'Поворот машин: вправо -> вниз',
        'turn_point_horizontal_left': 'Поворот машин: влево -> вверх',
        'turn_point_vertical_down': 'Поворот машин: вниз -> влево',
        'turn_point_vertical_up': 'Поворот машин: вверх -> вправо',
        'crosswalk_horizontal': 'Переход: через горизонтальную дорогу',
        'crosswalk_vertical': 'Переход: через вертикальную дорогу',
        'pedestrian_spawn_vertical': 'Старт пешеходов: вертикальный переход',
        'pedestrian_spawn_horizontal': 'Старт пешеходов: горизонтальный переход',
    }
    ...
    CONFIG_TABLE_EXCLUDED = {
        ...
        'TURN_POINT_HORIZONTAL_RIGHT_X',
        'TURN_POINT_HORIZONTAL_RIGHT_Y',
        'TURN_POINT_HORIZONTAL_LEFT_X',
        'TURN_POINT_HORIZONTAL_LEFT_Y',
        'TURN_POINT_VERTICAL_DOWN_X',
        'TURN_POINT_VERTICAL_DOWN_Y',
        'TURN_POINT_VERTICAL_UP_X',
        'TURN_POINT_VERTICAL_UP_Y',
        'HORIZONTAL_LANE_Y',
        'VERTICAL_LANE_X',
    ...
    CONFIG_LABELS = {
        ...
        'TURN_PROBABILITY': 'Вероятность поворота машин',
        'TURN_RADIUS': 'Радиус поворота машин',
        'TURN_WAIT_DISTANCE': 'Дистанция ожидания перед поворотом',
    ...
    VEHICLE_TURN_FIELDS = {'TURN_RADIUS', 'TURN_WAIT_DISTANCE', 'TURN_PROBABILITY'}
    ...
    def __init__(self):
        ...
        self.turn_points = []
        self.turn_point_map = {}
        self.crosswalk_markers = []
        self.crosswalk_marker_map = {}
        self.pedestrian_spawn_points = []
        self.pedestrian_spawn_point_map = {}
    ...
    def _register_turn_point(self, name, x, y, label, color):
        marker = EditableSpawnPointItem(
            x=x,
            y=y,
            name=name,
        ...
        self.scene.addItem(marker)
        self.turn_points.append(marker)
        self.turn_point_map[name] = marker
        return marker

    def _register_pedestrian_spawn_point(self, name, x, y, label, color):
        marker = EditableSpawnPointItem(
            x=x,
            y=y,
            name=name,
        ...
        self.scene.addItem(marker)
        self.pedestrian_spawn_points.append(marker)
        self.pedestrian_spawn_point_map[name] = marker
        return marker

    def _register_crosswalk_marker(self, name, x, y, width, height, label, color):
        marker = EditableCrosswalkItem(
            x=x,
            y=y,
            width=width,
            height=height,
        ...
        self.scene.addItem(marker)
        self.crosswalk_markers.append(marker)
        self.crosswalk_marker_map[name] = marker
        return marker
    ...
    def add_turn_markers(self):
        for marker in self.turn_points:
            if marker.scene() is self.scene:
                self.scene.removeItem(marker)
        self.turn_points.clear()
        self.turn_point_map.clear()

        self._register_turn_point(
            'turn_point_horizontal_right',
            self.config.TURN_POINT_HORIZONTAL_RIGHT_X,
            self.config.TURN_POINT_HORIZONTAL_RIGHT_Y,
            'T→↓',
            QColor(30, 160, 90),
        )
        self._register_turn_point(
            'turn_point_horizontal_left',
            self.config.TURN_POINT_HORIZONTAL_LEFT_X,
            self.config.TURN_POINT_HORIZONTAL_LEFT_Y,
            'T←↑',
            QColor(30, 120, 170),
        )
        self._register_turn_point(
            'turn_point_vertical_down',
            self.config.TURN_POINT_VERTICAL_DOWN_X,
            self.config.TURN_POINT_VERTICAL_DOWN_Y,
            'T↓←',
            QColor(120, 80, 200),
        )
        self._register_turn_point(
            'turn_point_vertical_up',
            self.config.TURN_POINT_VERTICAL_UP_X,
            self.config.TURN_POINT_VERTICAL_UP_Y,
            'T↑→',
            QColor(190, 90, 30),
        )

    def add_pedestrian_spawn_markers(self):
        for marker in self.pedestrian_spawn_points:
            if marker.scene() is self.scene:
                self.scene.removeItem(marker)
        self.pedestrian_spawn_points.clear()
        self.pedestrian_spawn_point_map.clear()

        self._register_pedestrian_spawn_point(
            'pedestrian_spawn_vertical',
            self.config.PEDESTRIAN_SPAWN_VERTICAL_X_MIN,
            self.config.PEDESTRIAN_SPAWN_VERTICAL_Y,
            'P↕',
            QColor(50, 205, 50),
        )
        self._register_pedestrian_spawn_point(
            'pedestrian_spawn_horizontal',
            self.config.PEDESTRIAN_SPAWN_HORIZONTAL_X,
            self.config.PEDESTRIAN_SPAWN_HORIZONTAL_Y_MIN,
            'P↔',
            QColor(0, 180, 120),
        )

    def add_crosswalk_markers(self):
        for marker in self.crosswalk_markers:
            if marker.scene() is self.scene:
                self.scene.removeItem(marker)
        self.crosswalk_markers.clear()
        self.crosswalk_marker_map.clear()

        self._register_crosswalk_marker(
            'crosswalk_horizontal',
            self.config.HORIZONTAL_CROSSWALK_X,
            self.config.HORIZONTAL_LANE_Y - 15,
            self.config.CROSSWALK_WIDTH,
            self.config.CROSSWALK_LENGTH,
            'CW↕',
            QColor(255, 255, 255),
        )
        self._register_crosswalk_marker(
            'crosswalk_vertical',
            self.config.VERTICAL_LANE_X - 15,
    ...
    def rebuild_editor_items(self):
        ...
        try:
            ...
            self.add_turn_markers()
            self.add_crosswalk_markers()
            self.add_pedestrian_spawn_markers()
    ...
    def get_editable_items(self):
        return (
            ...
            + list(self.spawn_points)
            + list(self.turn_points)
            + list(self.crosswalk_markers)
            + list(self.pedestrian_spawn_points)
        )
    ...
    def _sync_item_back_to_config(self, item):
        ...
        export_mapping = {
            ...
            'pedestrian_spawn_vertical': ('PEDESTRIAN_SPAWN_VERTICAL_X_MIN', 'PEDESTRIAN_SPAWN_VERTICAL_Y', None),
            'pedestrian_spawn_horizontal': ('PEDESTRIAN_SPAWN_HORIZONTAL_X', 'PEDESTRIAN_SPAWN_HORIZONTAL_Y_MIN', None),
            'turn_point_horizontal_right': ('TURN_POINT_HORIZONTAL_RIGHT_X', 'TURN_POINT_HORIZONTAL_RIGHT_Y', None),
            'turn_point_horizontal_left': ('TURN_POINT_HORIZONTAL_LEFT_X', 'TURN_POINT_HORIZONTAL_LEFT_Y', None),
            'turn_point_vertical_down': ('TURN_POINT_VERTICAL_DOWN_X', 'TURN_POINT_VERTICAL_DOWN_Y', None),
            'turn_point_vertical_up': ('TURN_POINT_VERTICAL_UP_X', 'TURN_POINT_VERTICAL_UP_Y', None),
        }

        if item.name == 'crosswalk_horizontal':
            self.config.HORIZONTAL_CROSSWALK_X = round(item.x())
            self.config.HORIZONTAL_LANE_Y = round(item.y() + 15)
            self.add_crosswalks()
            return

        if item.name == 'crosswalk_vertical':
            self.config.VERTICAL_LANE_X = round(item.x() + 15)
            self.config.VERTICAL_CROSSWALK_Y = round(item.y())
            self.add_crosswalks()
            return
    ...
    def _make_config_spec(self, name: str, value):
        ...
        if isinstance(value, float):
            ...
            if name == 'TURN_PROBABILITY':
                spec.update({'min': 0.0, 'max': 1.0, 'step': 0.05, 'decimals': 2})
            elif 'SPEED' in name:
        ...
        elif name in {'TURN_RADIUS', 'TURN_WAIT_DISTANCE'}:
            spec.update({'min': 0, 'max': 500, 'step': 1})
    ...
    def apply_config_value(self, name: str, value):
        ...
        if name in self.VEHICLE_TURN_FIELDS:
            self._refresh_vehicle_items_from_config(refresh_speed=False)

        if name in self.PEDESTRIAN_SPEED_FIELDS:
            self._refresh_pedestrian_items_from_config(refresh_speed=True)

        if name.startswith('PEDESTRIAN_SPAWN_') and self._editor_enabled:
            self.rebuild_editor_items()

        self.update_traffic_light_display()
        self.notify_editor()

    def save_config_to_file(self, file_path: str | None = None):
        """Сохраняет текущие числовые/логические настройки обратно в config.py."""
        target = Path(file_path) if file_path else Path(__file__).with_name('config.py')
        source = target.read_text(encoding='utf-8')

        values = {}
        for name, value in vars(self.config).items():
            if isinstance(value, bool):
                values[name] = _format_scalar_for_export(value)
        ...
        changed = 0
        for name, value_text in values.items():
            pattern = re.compile(rf'^(\s*{re.escape(name)}\s*:\s*(?:int|float|bool)\s*=\s*).+$', re.MULTILINE)
            source, count = pattern.subn(rf'\g<1>{value_text}', source)
            changed += count

        target.write_text(source, encoding='utf-8')
        return changed, str(target)
    ...
    def get_config_export_text(self):
        ...
        pedestrian_spawn_mapping = {
            'pedestrian_spawn_vertical': ('PEDESTRIAN_SPAWN_VERTICAL_X_MIN', 'PEDESTRIAN_SPAWN_VERTICAL_Y'),
            'pedestrian_spawn_horizontal': ('PEDESTRIAN_SPAWN_HORIZONTAL_X', 'PEDESTRIAN_SPAWN_HORIZONTAL_Y_MIN'),
        }
        lines.append('# Pedestrian spawn points')
        for name in pedestrian_spawn_mapping:
            item = self.pedestrian_spawn_point_map.get(name)
            if item is None:
                continue
            x_key, y_key = pedestrian_spawn_mapping[name]
            lines.append(f'{x_key}: int = {round(item.x())}')
            lines.append(f'{y_key}: int = {round(item.y())}')
            lines.append('')

        lines.append('# Crosswalk logic points')
        horizontal_crosswalk = self.crosswalk_marker_map.get('crosswalk_horizontal')
        if horizontal_crosswalk is not None:
            lines.append(f'HORIZONTAL_CROSSWALK_X: int = {round(horizontal_crosswalk.x())}')
            lines.append(f'HORIZONTAL_LANE_Y: int = {round(horizontal_crosswalk.y() + 15)}')
            lines.append('')
        vertical_crosswalk = self.crosswalk_marker_map.get('crosswalk_vertical')
        if vertical_crosswalk is not None:
            lines.append(f'VERTICAL_LANE_X: int = {round(vertical_crosswalk.x() + 15)}')
            lines.append(f'VERTICAL_CROSSWALK_Y: int = {round(vertical_crosswalk.y())}')
            lines.append('')

        turn_mapping = {
            'turn_point_horizontal_right': ('TURN_POINT_HORIZONTAL_RIGHT_X', 'TURN_POINT_HORIZONTAL_RIGHT_Y'),
            'turn_point_horizontal_left': ('TURN_POINT_HORIZONTAL_LEFT_X', 'TURN_POINT_HORIZONTAL_LEFT_Y'),
            'turn_point_vertical_down': ('TURN_POINT_VERTICAL_DOWN_X', 'TURN_POINT_VERTICAL_DOWN_Y'),
            'turn_point_vertical_up': ('TURN_POINT_VERTICAL_UP_X', 'TURN_POINT_VERTICAL_UP_Y'),
        }
        lines.append('# Vehicle turn points')
        for name in turn_mapping:
            item = self.turn_point_map.get(name)
            if item is None:
                continue
            x_key, y_key = turn_mapping[name]
            lines.append(f'{x_key}: int = {round(item.x())}')
            lines.append(f'{y_key}: int = {round(item.y())}')
            lines.append('')
    ...
    def add_pedestrian(self, direction='vertical'):
        ...
        if direction == 'vertical':
            x_min = int(self.config.PEDESTRIAN_SPAWN_VERTICAL_X_MIN)
            x_max = int(self.config.HORIZONTAL_CROSSWALK_X + self.config.CROSSWALK_WIDTH - self.config.PEDESTRIAN_WIDTH)
            low, high = sorted((x_min, x_max))
            x = random.randint(low, high)
            y = self.config.PEDESTRIAN_SPAWN_VERTICAL_Y
        else:
            x = self.config.PEDESTRIAN_SPAWN_HORIZONTAL_X
            y_min = int(self.config.PEDESTRIAN_SPAWN_HORIZONTAL_Y_MIN)
            y_max = int(
                self.config.VERTICAL_CROSSWALK_Y
                + self.config.CROSSWALK_WIDTH
